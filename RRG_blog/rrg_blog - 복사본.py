...
for i in range(len(tickers)):
    ...
    if len(rsr_tickers[i]) > 0 and len(rsm_tickers[i]) > 0:
        ...
        if len(rsr_tickers[i]) > 2:
            plt.scatter(rsr_tickers[i].iloc[1:-1], rsm_tickers[i].iloc[1:-1], s=20, label=tickers[i])
        ...
        # 연결선 그리기
        plt.plot(rsr_tickers[i], rsm_tickers[i], color=bg_color, marker='o', linewidth=1)

        # 각 종목의 마지막 데이터 지점에 화살표 추가
        for j in range(1, len(rsr_tickers[i])):
            plt.annotate(
                ...
                arrowprops=dict(arrowstyle='->', color=bg_color, lw=1.5)
            )
    else:
