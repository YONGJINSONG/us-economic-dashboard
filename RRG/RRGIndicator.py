...
def remove_ticker():
    ...
    tk.Label(main_frame, text="Select ticker to remove:", font=('Arial', 12, 'bold')).pack(pady=(0, 10))
    ...
    def remove_selected_ticker():
        ...
        if selection:
            ...
            ticker_name = tickers_metadata_dict['name'][index]
            
            # Show confirmation dialog
            confirm = tk.messagebox.askyesno(
                "Confirm Removal", 
                f"Are you sure you want to remove '{ticker_to_remove} - {ticker_name}'?"
            )
            
            if confirm:
                try:
                    # Remove from tickers_data
                    if ticker_to_remove in tickers_data.columns:
                        tickers_data = tickers_data.drop(columns=[ticker_to_remove])
                    ...
                    if index < len(scatter_plots):
                        ...
                        if index < len(arrow_annotations):
                    ...
                    tk.messagebox.showerror("Error", f"Failed to remove ticker: {str(e)}")
        else:
    ...
    # Add buttons
    remove_btn = tk.Button(button_frame, text="Remove", command=remove_selected_ticker, 
                          font=('Arial', 11, 'bold'), bg='lightcoral', fg='white', width=10)
    remove_btn.pack(side=tk.LEFT, padx=(0, 10))
    
    cancel_btn = tk.Button(button_frame, text="Cancel", command=cancel_remove, 
                          font=('Arial', 11), bg='lightgray', width=10)
...
def animate(i):
    ...
    for j in range(len(tickers)):
        ...
        if tickers[j] not in tickers_to_show:
            ...
            # Update the scatter
            if len(filtered_rsr_tickers.values) > 0 and len(filtered_rsm_tickers.values) > 0:
                color = get_color(filtered_rsr_tickers.values[-1], filtered_rsm_tickers.values[-1])
            else:
                color = 'gray'
            # Create appropriate marker sizes for the data points
            data_size = len(filtered_rsr_tickers.values)
            if data_size > 0:
                # Use appropriate marker sizes based on available data
                if data_size <= len(marker_size):
                    current_marker_size = marker_size[:data_size]
                else:
                    # If we have more data points than marker_size array, extend it
                    current_marker_size = marker_size + [10] * (data_size - len(marker_size))
                scatter_plots[j] = ax[0].scatter(filtered_rsr_tickers.values, filtered_rsm_tickers.values, color=color, s=current_marker_size)
            else:
                scatter_plots[j] = ax[0].scatter([], [], color=color)
            # Update the line
            ...
            if len(filtered_rsr_tickers.values) > 0 and len(filtered_rsm_tickers.values) > 0:
                ...
                annotations[j] = ax[0].annotate('', (0, 0))
